...
void Server::sendFrame()
{
    ...
    while ((cl = rfbClientIteratorNext(it)))
    {
        ...
        if (calcFrameCRC)
        {
            if (frame_crc == -1)
            {
                /* JFIF header contains some varying data so skip it for checksum calculation */
                frame_crc = boost::crc<32, 0x04C11DB7, 0xFFFFFFFF, 0xFFFFFFFF, true, true>(data + 0x30, video.getFrameSize() - 0x30);
            }
