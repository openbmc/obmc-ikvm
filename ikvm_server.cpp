...
Server::Server(const Args& args, Input& i, Video& v) :
    ...
    timeoutSeconds(args.getTimeoutSeconds())
{
...
void Server::sendFrame()
{
    ...
    while ((cl = rfbClientIteratorNext(it)))
    {
        ...
        if (timeoutSeconds >= 0)
        {
            auto inactivityDuration = std::chrono::duration_cast<std::chrono::seconds>(
                now - cd->lastActivityTime);

            if (inactivityDuration.count() > timeoutSeconds)
            {
                lg2::info("Closing idle client connection after {DURATION} seconds",
