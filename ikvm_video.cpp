...
void Video::getFrame()
{
    ...
    for (unsigned int i = 0; i < buffers.size(); ++i)
    {
        ...
        if (!buffers[i].queued)
        {
            ...
            if (rc)
            {
                lg2::error("Failed to queue buffer {ERROR}", "ERROR"s,
                           strerror(errno));
            }
            else
            {
...
bool Video::needsResize()
{
    ...
    if (rc < 0)
    {
        if (!timingsError)
        {
            lg2::error("Failed to query timings {ERROR}", "ERROR"s,
                       strerror(errno));
            timingsError = true;
        }
...
void Video::resize()
{
    ...
    if (needsResizeCall)
    {
        ...
        if (rc)
        {
            lg2::error("Failed to stop streaming {ERROR}", "ERROR"s,
                       strerror(errno));
            elog<ReadFailure>(
    ...
    if (needsResizeCall)
    {
        ...
        if (rc < 0)
        {
            lg2::error("Failed to zero streaming buffers {ERROR}", "ERROR"s,
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
        ...
        if (rc < 0)
        {
            lg2::error("Failed to query timings, restart {ERROR}", "ERROR"s,
                       strerror(errno));
            restart();
            return;
        }
        ...
        if (rc < 0)
        {
            lg2::error("Failed to set timings {ERROR}", "ERROR"s,
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
    ...
    if (rc < 0 || req.count < 2)
    {
        lg2::error("Failed to request streaming buffers {ERROR}", "ERROR"s,
                   strerror(errno));
        elog<ReadFailure>(
    ...
    for (i = 0; i < buffers.size(); ++i)
    {
        ...
        if (rc < 0)
        {
            lg2::error("Failed to query buffer {ERROR}", "ERROR"s,
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
        ...
        {
            lg2::error("Failed to mmap buffer {ERROR}", "ERROR"s,
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
    ...
    if (rc)
    {
        lg2::error("Failed to start streaming {ERROR}", "ERROR"s,
                   strerror(errno));
        elog<ReadFailure>(
            xyz::openbmc_project::Common::Device::ReadFailure::CALLOUT_ERRNO(
                errno),
...
void Video::start()
{
    ...
    if (fd < 0)
    {
        lg2::error("Failed to open video device {PATH} {ERROR}", "PATH"s,
                   path.c_str(), "ERROR"s, strerror(errno));
        elog<Open>(
    ...
    if (rc < 0)
    {
        lg2::error("Failed to query video device format {ERROR}", "ERROR"s,
                   strerror(errno));
        elog<ReadFailure>(
            xyz::openbmc_project::Common::Device::ReadFailure::CALLOUT_ERRNO(
                errno),
    ...
    if (rc < 0)
    {
        lg2::warning("Failed to set video device frame rate {ERROR}", "ERROR"s,
                     strerror(errno));
    }
    ...
    if (rc < 0)
    {
        lg2::warning("Failed to set video jpeg subsampling {ERROR}", "ERROR"s,
                     strerror(errno));
    }
    ...
    {
        lg2::error("Pixel Format not supported {PIXELFORMAT}", "PIXELFORMAT"s,
                   pixelformat);
    }
