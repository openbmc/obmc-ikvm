...
void Video::getFrame()
{
    ...
    for (unsigned int i = 0; i < buffers.size(); ++i)
    {
        ...
        if (!buffers[i].queued)
        {
            ...
            if (rc)
            {
                lg2::error("Failed to queue buffer {ERROR}", "ERROR",
                           strerror(errno));
            }
            else
            {
...
void Video::resize()
{
    ...
    if (needsResizeCall)
    {
        ...
        if (rc)
        {
            lg2::error("Failed to stop streaming {ERROR}", "ERROR",
                       strerror(errno));
            elog<ReadFailure>(
    ...
    if (needsResizeCall)
    {
        ...
        if (rc < 0)
        {
            lg2::error("Failed to zero streaming buffers {ERROR}", "ERROR",
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
        ...
        if (rc < 0)
        {
            lg2::error("Failed to query timings, restart {ERROR}", "ERROR",
                       strerror(errno));
            restart();
            return;
        }
    ...
    for (i = 0; i < buffers.size(); ++i)
    {
        ...
        if (rc < 0)
        {
            lg2::error("Failed to query buffer {ERROR}", "ERROR",
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
        ...
        {
            lg2::error("Failed to mmap buffer {ERROR}", "ERROR",
                       strerror(errno));
            elog<ReadFailure>(
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_ERRNO(errno),
    ...
    if (rc)
    {
        lg2::error("Failed to start streaming {ERROR}", "ERROR",
                   strerror(errno));
        elog<ReadFailure>(
            xyz::openbmc_project::Common::Device::ReadFailure::CALLOUT_ERRNO(
                errno),
...
void Video::start()
{
    ...
    if (rc < 0)
    {
        lg2::error("Failed to query video device format {ERROR}", "ERROR",
                   strerror(errno));
        elog<ReadFailure>(
            xyz::openbmc_project::Common::Device::ReadFailure::CALLOUT_ERRNO(
                errno),
    ...
    if (rc < 0)
    {
        lg2::warning("Failed to set video device frame rate {ERROR}", "ERROR",
                     strerror(errno));
    }
    ...
    if (rc < 0)
    {
        lg2::warning("Failed to set video jpeg subsampling {ERROR}", "ERROR",
                     strerror(errno));
    }
