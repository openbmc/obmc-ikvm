...
void Input::connect()
{
    ...
    {
        lg2::error("Failed to search USB virtual hub port {ERROR}", "ERROR"s,
                   e.what());
        return;
    }
    catch (std::ofstream::failure& e)
    {
        lg2::error("Failed to connect HID gadget {ERROR}", "ERROR"s, e.what());
        return;
    }
    ...
    if (!keyboardPath.empty())
    {
        ...
        if (keyboardFd < 0)
        {
            lg2::error("Failed to open input device {PATH} {ERROR}", "PATH"s,
                       keyboardPath.c_str(), "ERROR"s, strerror(errno));
    ...
    if (!pointerPath.empty())
    {
        ...
        {
            lg2::error("Failed to open input device {PATH} {ERROR}", "PATH"s,
                       pointerPath.c_str(), "ERROR"s, strerror(errno));
            elog<Open>(xyz::openbmc_project::Common::File::Open::ERRNO(errno),
                       xyz::openbmc_project::Common::File::Open::PATH(
                           pointerPath.c_str()));
        }
...
void Input::disconnect()
{
    ...
    {
        lg2::error("Failed to disconnect HID gadget {ERROR}", "ERROR"s,
                   e.what());
    }
}
...
bool Input::writeKeyboard(const uint8_t* report)
{
    ...
    while (retryCount > 0)
    {
        ...
        if (errno != EAGAIN)
        {
            if (errno != ESHUTDOWN)
            {
                lg2::error("Failed to write keyboard report {ERROR}", "ERROR"s,
                           strerror(errno));
            }
...
void Input::writePointer(const uint8_t* report)
{
    ...
    while (retryCount > 0)
    {
        ...
        if (errno != EAGAIN)
        {
            if (errno != ESHUTDOWN)
            {
                lg2::error("Failed to write pointer report {ERROR}", "ERROR"s,
                           strerror(errno));
            }
